...
if __name__ == "__main__":
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Image Processing Results"

    # 寫入表頭
    headers = ["Filename", "Processing Time", "Original Area", "Convex Hull Area", "Area Ratio", 
               "Original Circularity", "Convex Hull Circularity", "Circularity Ratio"]
    ws.append(headers)
        ...
        if filename.endswith('.tiff') and filename != 'background.tiff':
            ...
            if results:
                ...
                row = [
                    filename,
                    process_time,
                    results['area_original'],
                    results['area_hull'],
                    results['area_ratio'],
                    results['circularity_original'],
                    results['circularity_hull'],
                    results['circularity_ratio']
                ]
                ws.append(row)
    ...
    if image_count > 0:
        ...
        # 將平均處理時間添加到Excel
        ws.append([])  # 添加一個空行
        ws.append(["Average processing time per image", average_time])

    # 保存Excel文件
    excel_filename = "image_processing_results.xlsx"
